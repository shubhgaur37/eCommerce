...
This journal reconstructs the project chronologically from its Git history.
It records what each meaningful change introduced, why it matters, what was
learned while implementing it, and where the current design has production
trade-offs. Merge-only commits and documentation-only rewrites are noted
briefly because they did not change runtime behavior.

## 20 September 2024 — Building the synchronous foundation

### Part 1: Order and inventory services (`4e126cd`)

The project began with two independent Spring Boot applications. Inventory
owned `Product`, stock, price, its repository, REST endpoints, and PostgreSQL
configuration. Order owned `Orders`, `OrderItem`, order status, its repository,
and its own endpoints and database configuration. DTOs were kept separate from
JPA entities, and ModelMapper handled basic boundary mapping.

Key learnings:

- A microservice boundary starts with ownership. Order and inventory should not
  share database tables or persistence entities merely because their data is
  related.
- DTOs keep HTTP contracts separate from storage models. This allows either
  side to evolve without automatically exposing every persistence detail.
- Each service can be built and deployed independently because each has its own
  Maven wrapper and Spring Boot entry point.
- `ddl-auto=create-drop` and seed SQL are convenient for learning but destructive
  and unsuitable for durable environments. Real deployments need migrations
  such as Flyway or Liquibase and persistent data policies.
- Mapping libraries reduce boilerplate, but convention-based mapping must be
  verified when fields have similar names and different semantics.

### Part 2: Eureka service discovery (`d8ea76a`)

A dedicated Eureka server was added, and order and inventory became discovery
clients. Their controllers also gained endpoints used to exercise calls across
service boundaries.

Key learnings:

- Discovery replaces hard-coded instance addresses with logical service names.
  This is especially useful when instances scale or move.
- Registration and discovery do not make remote calls reliable. A discovered
  service can still be unavailable, overloaded, or fail midway through work.
- The registry itself becomes platform infrastructure. Its availability,
  startup order, and health affect the rest of the system.

### Part 3: API Gateway (`363088c`)

Spring Cloud Gateway was introduced as the system's edge. Routes used
load-balanced `lb://` destinations resolved through Eureka, and path rewriting
allowed clients to call stable external paths while internal services retained
their own context paths.

Key learnings:

- A gateway centralizes cross-cutting concerns and hides internal topology from
  clients.
- Route predicates decide which traffic matches; filters can then transform
  paths, headers, authentication context, or responses.
- Gateway routes and service context paths must be designed together. A wrong
  `StripPrefix` count produces valid routing to the wrong downstream URL.
- The gateway is not a substitute for service authorization or network
  isolation; direct access to internal services must still be controlled.

### Part 4: OpenFeign communication (`96b3929`)

Both business services gained declarative Feign clients. Order could request an
inventory reservation, and inventory could call order-related operations using
service names resolved through discovery. Order creation moved beyond local
database work into a synchronous distributed interaction.

Key learnings:

- Feign removes repetitive HTTP-client code, but it does not remove network
  latency, timeouts, partial failure, or versioned API contracts.
- Request and response DTOs are wire contracts. Duplicating them across modules
  is simple initially but creates drift risk as the system grows.
- A database transaction cannot automatically span an HTTP call to another
  service. Each service commits independently, so compensating behavior or a
  different consistency model may be required.

### Part 5: Resilience4J (`c2a58cb`)

The order-to-inventory call gained Resilience4J integration, including an
active circuit breaker and fallback. Retry and rate-limiter experiments were
also introduced in the service code/configuration.

Key learnings:

- A circuit breaker stops repeatedly calling a dependency that is already
  failing and periodically probes recovery in half-open state.
- Retry is appropriate only for transient, idempotent operations. Retrying a
  non-idempotent stock reservation can duplicate side effects.
- A rate limiter protects capacity; it does not detect downstream health.
- Fallback signatures must match the protected method plus the expected
  throwable, and fallbacks should expose degraded behavior rather than silently
  pretending the original operation succeeded.
- Combining retries, circuit breakers, and timeouts requires deliberate order
  and thresholds; careless retries can amplify an outage.

## 26 September 2024 — Gateway behavior and security

### Part 6: Global and route filters (`e972c26`)

The gateway gained `GlobalLoggingFilter`, which observes every routed request,
and `LoggingOrdersFilter`, a named filter intended only for configured routes.
Pre-filter logic logged incoming URIs, while the global post stage logged the
resulting response status.

Key learnings:

- Global filters apply across the gateway; a custom
  `AbstractGatewayFilterFactory` applies only where a route declares it.
- Reactive post-processing belongs after `chain.filter(exchange)`, commonly
  through operators such as `then` or `doFinally`.
- Filter order controls whether logging, authentication, mutation, and routing
  run before or after one another.
- Class naming matters: Spring derives the configuration filter name from the
  factory class name by removing `GatewayFilterFactory`.

### Part 7: JWT authentication at the gateway (`ab61c97`)

A configurable authentication filter and JWT service were added. The filter
read a Bearer token, verified its signature, extracted the subject as user ID,
and forwarded identity through `X-User-Id`. Routes could turn the filter on or
off through configuration.

Key learnings:

- Authentication answers who the caller is; authorization still needs to
  decide what that identity may do.
- Bearer-header parsing needs validation for missing, malformed, or invalid
  values. Splitting blindly can turn client errors into server errors.
- Downstream services should trust identity headers only when clients cannot
  bypass the gateway or spoof those headers.
- Signing keys belong in secret storage or injected environment configuration,
  never source control.
- In a reactive gateway, request mutation must be attached to the exchanged
  request correctly; creating a mutated builder without using the resulting
  exchange can leave the forwarded request unchanged.

## 27 September 2024 — Central configuration and observability

### Part 8: Spring Cloud Config (`8186197`)

A Config Server was introduced on port `8888`. Gateway, order, and inventory
became config clients, and most environment-specific settings moved to a
separate Git repository. The clients retained the bootstrap information they
need before remote configuration can be fetched: `spring.application.name`
and `spring.config.import`.

Key learnings:

- Configuration cannot be fully externalized: clients must locally know their
  identity and how to find the Config Server.
- A required `configserver:` import intentionally makes configuration
  availability a startup dependency. An `optional:` import changes that
  failure behavior and should be chosen consciously.
- Central configuration makes environment changes consistent and auditable,
  but it adds Git access, credentials, network availability, and startup order
  as operational concerns.
- Service names determine which remote property sources a client requests, so
  naming mismatches can look like missing configuration.

### Part 8 continued: Refresh scope (`1bb7e87`)

A feature configuration bean was added under `@RefreshScope`, and controller
behavior began reading remotely supplied values at runtime.

Key learnings:

- A normal singleton captures injected values when it is created. Refreshing
  the environment alone does not reconstruct that singleton.
- `@RefreshScope` belongs on the bean that owns the changing values. Callers
  receive a scoped proxy and do not all need the annotation themselves.
- Refresh requires the Actuator endpoint to be exposed and invoked; changing
  the Git repository does not instantly update running processes by itself.
- A distributed feature flag must be refreshed consistently in every service
  participating in that protocol.

### Part 9: Zipkin distributed tracing (`068dfc0`)

Micrometer tracing, Brave propagation, and the Zipkin reporter were added to
the gateway and business services. This allowed one request to be correlated
across gateway routing and synchronous service calls.

Key learnings:

- A trace represents an end-to-end operation; spans represent individual work
  such as gateway routing, HTTP calls, database work, or message handling.
- Trace context must be propagated across process boundaries. Dependencies
  provide instrumentation, but sampling and exporter settings still determine
  which spans reach Zipkin.
- Trace IDs in logs bridge two observability views: timing/topology in Zipkin
  and detailed application context in logs.

### Part 10: Rolling service logs (`c60f259`)

Logback configuration was added to gateway, inventory, and order. Services
wrote rolling files into service-specific directories beneath `logs/`, creating
the source that Logstash would later ingest.

Key learnings:

- Central logging starts with predictable output: consistent patterns,
  service identity, trace IDs, retention, and rollover.
- Committed runtime logs are noisy, can contain sensitive data, and should
  normally be ignored. The later working tree removes old tracked examples.
- Logs and traces complement rather than replace one another.

## 3 September 2026 — Hardening configuration and adding ELK
...
## 4 September 2026 — Kafka and the event-driven flow
...
### Dedicated learning section (`075a2bc`)

The main README gained a consolidated learning section. This current
`learnings/` directory completes that separation: the root README now explains
the project as it exists, while this journal preserves how and why it evolved.

## 5 September 2026 — Migrating event contracts to Avro
